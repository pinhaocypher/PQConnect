...
class TestKeyGen(TestCase):
    def setUp(self) -> None:
        ...
        # Profile the main functions we want to test
        self.profiler.add_function(save_keys)
        self.profiler.add_function(static_keygen)

    @mock.patch("builtins.input", return_value="y")
    def test_save_keys_create_dir(self, mocked_input: Mock) -> None:
        with self.cli.isolated_filesystem():
            self.profiler.runcall(save_keys, "some_path", b"0", b"1", b"2", b"3", b"4")
    ...
    def test_save_keys_create_dir_decline(self, mocked_input: Mock) -> None:
        with self.cli.isolated_filesystem():
            self.profiler.runcall(save_keys, "some_path_new", b"0", b"1", b"2", b"3", b"4")
            self.profiler.print_stats()
            self.assertFalse(
    ...
    def test_file_perms(self, mocked_input: Mock) -> None:
        ...
        path = "some_path"
        with self.cli.isolated_filesystem():
            self.profiler.runcall(save_keys, path, b"0", b"1", b"2", b"3", b"4")
            self.profiler.print_stats()
    ...
    def test_static_keygen(self) -> None:
        ...
        with self.cli.isolated_filesystem():
            self.profiler.runcall(static_keygen, ".", 12345, 54321, False)
            self.profiler.print_stats()
    ...
    def test_static_keygen_fail(self, mocked: Mock) -> None:
        ...
        self.profiler.runcall(static_keygen, path, pqcport, keyport, dns_only)
        self.profiler.print_stats()
        self.assertFalse(
